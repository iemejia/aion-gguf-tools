...
- `/tmp/aion-q40.gguf` - default model produced directly by the converter, about 1.4 GB.
- `/tmp/aion-q4_k_m.gguf` - separately quantized benchmark model, 1.6 GB.
...
```sh
AION_GGUF=/tmp/aion-q4_k_m.gguf scripts/aion_demo.sh "Hello"
...
```sh
python scripts/aion_validate_gguf.py --model-dir /path/to/aion-onnx-bundle --gguf /tmp/aion-q40.gguf --allow-quantized --bench-prompt-tokens 128 --bench-gen-tokens 64 --bench-repetitions 3
...
Historical benchmark results on the current M4 Max:
...
The demo wrapper uses the same safe knobs by default plus `AION_CTX=2048`, which reduces KV memory from about 896 MiB at 8192 context to about 224 MiB. Raise `AION_CTX` if a demo prompt needs more context.
